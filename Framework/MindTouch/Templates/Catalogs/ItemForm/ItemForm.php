{{
  ...
  <eval:if test="current is nil">
    <ul class="ae_errors">
      <li class="ae_error">Not set basepath to ItemForm template</li>
    </ul>
  </eval:if>
  ...
  <eval:else>
    <pre class="script">
      ...
      foreach (var tabular in tabulars) {
         ...
         let options ..= { (tabular): { page: __request.args[ppName] ?? 1 }};
      }
      
      let data = entities.displayItemForm(uid, id, {options: options});
