	...
	this.drawEmployeeVacationDays = function(tag_id, data, options)
	{
		...
		google.setOnLoadCallback(function ()
		{
			...
			var opt = {
				...
				chl: spent,
				chma: '5',
				chf: 'bg,s,' + (options && options.chart && options.chart.background ? options.chart.background : 'FFFFFF')
			};
			vis.draw(dataTable, opt);
		});
		...
		google.setOnLoadCallback(function ()
		{
			...
			table.draw(data, options && options.table ? options.table : {});
		});
	};
}
